...
CollectorController::onSampleRate(
    ...
    if (switchMemoryProfilingOn) {
        if (memoryProfilingPushRateMillis != DONT_CHANGE_SAMPLE_RATE) {
            memoryProfilingPushRateMillis_ = memoryProfilingPushRateMillis;
        }
        if (!memoryProfilingOn_) {
            MemoryProfiler::start(switchMemoryProfilingStacktraceOn, memoryProfilingStackSampleRateSamples);
        }

        debugLogger_ << "scheduling allocation timer" << endl;
        scheduleAllocationTimer();
    } else if (memoryProfilingOn_) {
        MemoryProfiler::stop();
    }*/
...
void CollectorController::backoff() {
    ...
    terminator_.timedWait(randomisedBackoffTime);
}
