...
void Metrics::stopThread() {
    terminator_.terminate();
    debugLogger_ << "Stopping metrics" << endl;
...
void Metrics::run() {
    try {
        ...
        while (terminator_.isRunning()) {
            uint64_t durationInMs = sampleRateMillis_.load();

            terminator_.timedWait(durationInMs);
